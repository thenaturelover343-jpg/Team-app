...
async function activateWaitingWorker(registration: ServiceWorkerRegistration | null) {
  ...
  if (waiting) {
    ...
    await new Promise<void>((resolve) => {
      ...
      // Fallback if controllerchange never fires (Safari quirks)
...
/**
 * Polls /app-version.txt on interval + window focus.
 * First successful fetch seeds the running BUILD_ID; later mismatches → updateAvailable.
 */
export function useAppUpdate() {
  ...
  const applyUpdate = useCallback(async () => {
    ...
    try {
      ...
      // Hard reload clears stale RSC/Vite chunks in Safari + installed PWA
