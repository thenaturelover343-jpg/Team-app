...
export default function NotificationCenter({ notifications, push, onChanged }: { notifications: TeamNotification[]; push: PushState; onChanged: () => Promise<void> }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="bg-white border border-zinc-200 rounded-2xl p-5 space-y-4">
      ...
      {!pushSupported() && <p className="text-sm bg-amber-50 text-amber-800 border border-amber-200 p-3 rounded-xl">Installeer de website op het beginscherm en open hem daarna opnieuw om pushmeldingen op dit toestel te gebruiken.</p>}
      {message && <p className="text-sm bg-zinc-50 border border-zinc-200 p-3 rounded-xl">{message}</p>}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button onClick={activate} disabled={busy !== '' || !push.supported || !pushSupported()} className="bg-zinc-900 text-white rounded-xl py-3 font-bold disabled:opacity-50 flex items-center justify-center gap-2">{busy === 'enable' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Bell className="w-4 h-4" />}Meldingen activeren</button>
        {push.enabled && <button onClick={deactivate} disabled={busy !== ''} className="border border-zinc-300 rounded-xl py-3 font-bold disabled:opacity-50">Op dit toestel uitzetten</button>}
      </div>
    </section>
    ...
    <div className="space-y-3">
      {notifications.length === 0 && <div className="bg-white border border-zinc-200 rounded-xl p-5 text-zinc-500">Nog geen meldingen.</div>}
      {notifications.map(item => <button key={item.id} onClick={() => !item.readAt && markOne(item.id)} className={`w-full text-left border rounded-2xl p-4 transition-colors ${item.readAt ? 'bg-white border-zinc-200' : 'bg-blue-50 border-blue-200'}`}>
